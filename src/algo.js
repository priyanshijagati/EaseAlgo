...
const Algo = () => {
  const questions = [
    { 
      question: "1. A list of n string, each of length n, is sorted into lexicographic order using the merge-sort algorithm. The worst case running time of this computation is " ,
       options:
      [
       "O (n*log(n));",
       "O (n2 log n);",
      ...
      correctAnswer: "O (n2 log n);",
    },


     { 
      question: "2. In a modified merge sort, the input array is splitted at a position one-third of the length(N) of the array. Which of the following is the tightest upper bound on time complexity of this modified Merge Sort.",
      options: [
        "N(logN base 3)", 
        "N(logN base 2/3)", 
        "N(logN base 1/3)", 
      ...
      <div className="sidebar">
        ...
        <ul>
          ...
          {questions.slice(14, 21).map((_, index) => (
            <li key={index}>
              <a href={`#question-${index + 14}`} onClick={handleSmoothScroll}>
                Question {index + 15}
              </a>
